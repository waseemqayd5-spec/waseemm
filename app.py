...
يدعم SQLite (محلي) و PostgreSQL (إنتاج)
الألوان: أسود، ذهبي، أبيض
المميزات:
- نظام صلاحيات (مدير، كاشير، مخزني)
- فواتير مبيعات مع نقاط ولاء
- إدارة موردين ومشتريات
- عروض ديناميكية (كوبونات)
- تقارير متقدمة
- رفع صور منتجات
- إعدادات متجر
- تنبيهات نقص مخزون وصلاحية
- واجهة POS للكاشير
...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        cur.execute("INSERT INTO customers (phone, name, loyalty_points, total_spent, visits, last_visit, tier) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    ("0500000000", "عميل تجريبي", 100, 200.0, 5, datetime.date.today().isoformat(), "فضي"))
    ...
    if cur.fetchone()[0] == 0:
        ...
        products = [
            ("8801234567890", "أرز بسمتي", "مواد غذائية", 25.0, 18.0, 50, 10, "كيلو", 1, (today + datetime.timedelta(days=180)).isoformat()),
            ("8809876543210", "سكر", "مواد غذائية", 15.0, 11.0, 100, 20, "كيلو", 1, (today + datetime.timedelta(days=180)).isoformat()),
            ("8801122334455", "زيت دوار الشمس", "مواد غذائية", 35.0, 28.0, 30, 10, "لتر", 1, (today + datetime.timedelta(days=180)).isoformat()),
            ("8805566778899", "حليب طازج", "مبردات", 8.0, 6.0, 40, 15, "لتر", 1, (today + datetime.timedelta(days=14)).isoformat()),
            ("8809988776655", "شاي", "مواد غذائية", 20.0, 15.0, 60, 15, "علبة", 1, (today + datetime.timedelta(days=180)).isoformat())
        ]
        for prod in products:
            cur.execute("""
                INSERT INTO products (barcode, name, category, price, cost_price, quantity, min_quantity, unit, supplier_id, expiry_date, added_date, last_updated)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Tahoma', Arial, sans-serif; }
            ...
            .product-card { background: #000; border: 1px solid #FFD700; border-radius: 10px; padding: 15px; text-align: center; transition: 0.3s; }
            ...
            .product-price { color: #FFD700; font-size: 20px; font-weight: bold; margin: 10px 0; }
            .product-stock { font-size: 12px; color: #aaa; }
            button { background: #FFD700; color: #000; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-weight: bold; margin-top: 10px; transition: 0.2s; }
...
def products_page():
    ...
    <style>body{background:#000;color:#fff;padding:20px;}.container{max-width:1200px;margin:auto;}.header{background:#111;border:1px solid #FFD700;padding:20px;border-radius:10px;margin-bottom:20px;color:#FFD700;}.filters{display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;}.filters input,.filters select{background:#000;border:1px solid #FFD700;color:#FFD700;padding:8px;border-radius:5px;}.products-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:20px;}.product-card{background:#111;border:1px solid #FFD700;border-radius:10px;padding:15px;text-align:center;}.product-name{color:#FFD700;font-weight:bold;}.product-price{color:#FFD700;font-size:20px;margin:10px 0;}.nav a{color:#FFD700;text-decoration:none;margin-left:15px;}</style>
    </head>
    ...
    <script>
        function loadProducts(){let s=document.getElementById('search').value;let c=document.getElementById('category').value;fetch(`/api/products?search=${encodeURIComponent(s)}&category=${encodeURIComponent(c)}`).then(r=>r.json()).then(data=>{if(data.success){let html='';data.products.forEach(p=>{html+=`<div class="product-card"><div class="product-name">${p.name}</div><div class="product-price">${p.price} ريال</div><div>المتبقي: ${p.quantity}</div><button onclick="addToCart(${p.id},'${p.name}',${p.price})">أضف للسلة</button></div>`;});document.getElementById('products').innerHTML=html;}});}
        function addToCart(id,name,price){let cart=JSON.parse(localStorage.getItem('cart')||'[]');let ex=cart.find(i=>i.id==id);if(ex)ex.quantity++;else cart.push({id,name,price,quantity:1});localStorage.setItem('cart',JSON.stringify(cart));alert('تمت الإضافة');}
...
def cart_page():
    ...
    <style>body{background:#000;color:#fff;padding:20px;}.container{max-width:800px;margin:auto;}.cart-item{background:#111;border:1px solid #FFD700;border-radius:5px;padding:10px;margin:10px 0;display:flex;justify-content:space-between;align-items:center;}.total{background:#FFD700;color:#000;padding:15px;border-radius:5px;font-weight:bold;margin:20px 0;text-align:center;}.btn{background:#FFD700;color:#000;padding:10px;border:none;border-radius:5px;cursor:pointer;}.whatsapp{background:#25D366;}.checkout{background:#FFD700;}</style>
    </head>
    <body><div class="container"><h1>سلة المشتريات</h1><div id="cart-items"></div><div class="total" id="total">الإجمالي: 0 ريال</div><button class="btn whatsapp" onclick="sendWhatsApp()">إرسال الطلب عبر واتساب</button><button class="btn checkout" onclick="checkout()">إنهاء الطلب (كاشير)</button></div>
    <script>
        let cart=JSON.parse(localStorage.getItem('cart')||'[]');
        function render(){let html='';let total=0;cart.forEach((item,i)=>{let itemTotal=item.price*item.quantity;total+=itemTotal;html+=`<div class="cart-item"><div>${item.name} x${item.quantity}</div><div>${itemTotal} ريال</div><button onclick="removeItem(${i})">حذف</button></div>`;});document.getElementById('cart-items').innerHTML=html||'<p>السلة فارغة</p>';document.getElementById('total').innerText=`الإجمالي: ${total} ريال`;}
        function removeItem(idx){cart.splice(idx,1);localStorage.setItem('cart',JSON.stringify(cart));render();}
        function sendWhatsApp(){if(cart.length==0){alert('السلة فارغة');return;}let msg='*طلب جديد*%0A';let total=0;cart.forEach(item=>{let t=item.price*item.quantity;total+=t;msg+=`- ${item.name} x${item.quantity} = ${t} ريال%0A`;});msg+=`%0A*الإجمالي: ${total} ريال*`;window.open(`https://wa.me/967771602370?text=${msg}`,'_blank');}
        function checkout(){if(cart.length==0){alert('السلة فارغة');return;}localStorage.setItem('pending_cart',JSON.stringify(cart));window.location.href='/login';}
        render();
    </script>
    </body>
    """)
...
def offers_page():
    ...
    <script>
        fetch('/api/offers').then(r=>r.json()).then(data=>{let html='';data.offers.forEach(o=>{html+=`<div class="offer"><h3>${o.title}</h3><p>${o.description}</p><div>كود: ${o.code}</div></div>`;});document.getElementById('offers').innerHTML=html;});
    </script>
    </body>
    """)
...
def points_page():
    ...
    <body><div class="card"><h2>استعلام عن النقاط</h2><input type="tel" id="phone" placeholder="رقم الهاتف"><button onclick="check()">استعلام</button><div id="result"></div></div>
    <script>
...
def admin_dashboard():
    ...
    <style>body{background:#000;color:#FFD700;font-family:Arial;padding:20px;}.header{background:#111;border:1px solid #FFD700;padding:20px;border-radius:10px;margin-bottom:20px;}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:20px;}.card{background:#111;border:1px solid #FFD700;padding:20px;border-radius:10px;text-align:center;cursor:pointer;transition:0.3s;}.card:hover{transform:translateY(-5px);background:#222;}.logout{position:absolute;top:20px;left:20px;background:#FFD700;color:#000;padding:10px;border-radius:5px;text-decoration:none;}</style>
    </head>
...
def pos():
    ...
    <style>body{background:#000;color:#FFD700;font-family:Arial;padding:20px;}.container{display:grid;grid-template-columns:1fr 350px;gap:20px;}.products{background:#111;border:1px solid #FFD700;border-radius:10px;padding:15px;height:70vh;overflow-y:auto;}.cart{background:#111;border:1px solid #FFD700;border-radius:10px;padding:15px;}.search{width:100%;padding:8px;margin-bottom:10px;background:#000;color:#FFD700;border:1px solid #FFD700;}.product-item{background:#000;border:1px solid #FFD700;border-radius:5px;padding:8px;margin-bottom:5px;cursor:pointer;display:flex;justify-content:space-between;}.cart-item{display:flex;justify-content:space-between;margin:5px 0;padding:5px;border-bottom:1px solid #FFD700;}.total{background:#FFD700;color:#000;padding:10px;border-radius:5px;text-align:center;font-weight:bold;margin:10px 0;}.btn{background:#FFD700;color:#000;padding:10px;border:none;border-radius:5px;cursor:pointer;}</style>
    </head>
    ...
    <script>
            ...
            fetch(`/api/products?search=${encodeURIComponent(q)}`)
                ...
                .then(data=>{
                    if(data.success){
                        let html='';
                        data.products.forEach(p=>{
                            html+=`<div class="product-item" onclick="addToCart(${p.id},'${p.name}',${p.price})"><span>${p.name}</span><span>${p.price} ريال</span><span>${p.quantity}</span></div>`;
                        });
        ...
        function checkout(){
            ...
            let order = {cart, customer_phone: customerPhone, payment_method:'cash'};
...
def admin_products():
    ...
    <style>body{background:#000;color:#FFD700;padding:20px;}.btn{background:#FFD700;color:#000;padding:8px 15px;border:none;border-radius:5px;cursor:pointer;}.table{width:100%;border-collapse:collapse;}.table th,.table td{border:1px solid #FFD700;padding:8px;text-align:right;}.form-group{margin-bottom:10px;}.form-group label{display:block;margin-bottom:5px;}</style>
    </head>
    ...
    <div id="add-form" style="display:none; margin-top:20px; background:#111; padding:20px; border-radius:10px;">
        ...
        <form id="productForm" enctype="multipart/form-data">
            ...
            <div class="form-group"><label>الصورة</label><input type="file" name="image"></div>
...
def api_add_product():
    try:
        ...
        if image and allowed_file(image.filename):
            ...
            filename = f"{int(time.time())}_{name_parts[0]}.{name_parts[1]}"
        ...
        if DATABASE_URL:
            execute_query("""
                INSERT INTO products (barcode, name, category, price, cost_price, quantity, min_quantity, unit, supplier_id, expiry_date, added_date, last_updated, image_url)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (barcode, name, category, price, cost_price, quantity, min_quantity, unit, supplier_id, expiry_date, today, today, image_url), commit=True)
        else:
            execute_query("""
                INSERT INTO products (barcode, name, category, price, cost_price, quantity, min_quantity, unit, supplier_id, expiry_date, added_date, last_updated, image_url)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def api_offers():
    today = datetime.date.today().isoformat()
    if DATABASE_URL:
        rows = execute_query("SELECT * FROM offers WHERE is_active=1 AND (start_date IS NULL OR start_date <= %s) AND (end_date IS NULL OR end_date >= %s)", (today, today), fetch_all=True)
    else:
...
@admin_required
def admin_customers():
    rows = execute_query("SELECT id, name, phone, loyalty_points, total_spent, visits, last_visit, tier FROM customers WHERE is_active=1 ORDER BY total_spent DESC", fetch_all=True)
    html = '<!DOCTYPE html><html dir="rtl"><head><title>العملاء</title><style>body{background:#000;color:#FFD700;padding:20px;}table{border-collapse:collapse;}td,th{border:1px solid #FFD700;padding:8px;}</style></head><body><h1>العملاء</h1><table border="1"><tr><th>الاسم</th><th>الهاتف</th><th>النقاط</th><th>الإنفاق</th><th>الزيارات</th><th>آخر زيارة</th><th>المستوى</th></tr>'
    for r in rows:
        html += f"<tr><td>{r['name']}</td><td>{r['phone']}</td><td>{r['loyalty_points']}</td><td>{r['total_spent']}</td><td>{r['visits']}</td><td>{r['last_visit']}</td><td>{r['tier']}</td></tr>"
...
@admin_required
def admin_invoices():
    rows = execute_query("SELECT id, invoice_number, customer_name, total, final_total, created_at FROM invoices ORDER BY created_at DESC", fetch_all=True)
    html = '<!DOCTYPE html><html dir="rtl"><head><title>الفواتير</title><style>body{background:#000;color:#FFD700;padding:20px;}</style></head><body><h1>الفواتير</h1><table border="1"><tr><th>رقم الفاتورة</th><th>العميل</th><th>الإجمالي</th><th>النهائي</th><th>التاريخ</th></tr>'
    for r in rows:
        html += f"<tr><td>{r['invoice_number']}</td><td>{r['customer_name']}</td><td>{r['total']}</td><td>{r['final_total']}</td><td>{r['created_at']}</td></tr>"
    html += '</table><a href="/admin">العودة</a></body></html>'
    return html
...
def admin_reports():
    ...
    total_revenue = execute_query("SELECT SUM(final_total) FROM invoices", fetch_one=True)[0] or 0
    total_customers = execute_query("SELECT COUNT(*) FROM customers", fetch_one=True)[0]
    total_points = execute_query("SELECT SUM(loyalty_points) FROM customers", fetch_one=True)[0] or 0
    # أفضل المنتجات مبيعاً (من invoice_items)
    if DATABASE_URL:
    ...
    html += f'<p>عدد الفواتير: {total_invoices}</p><p>إجمالي الإيرادات: {total_revenue:.2f} ريال</p><p>إجمالي العملاء: {total_customers}</p><p>إجمالي النقاط: {total_points}</p>'
...
def api_create_invoice():
    ...
    customer = None
    if customer_phone:
        customer = execute_query("SELECT id, name, phone FROM customers WHERE phone = ?", (customer_phone,), fetch_one=True)
    if not customer:
        customer = {"id": None, "name": "عميل نقدي", "phone": ""}
    # حساب الإجمالي
    total = sum(item['price'] * item['quantity'] for item in cart)
    discount = 0
    # يمكن تطبيق كوبون هنا (لم يتم)
    ...
    if DATABASE_URL:
        ...
        """, (invoice_number, customer['id'], customer['name'], customer['phone'], total, discount, final_total, payment_method, session['user_id']), fetch_one=True, commit=True)
        invoice_id = row['id']
    else:
        ...
        cur.execute("""
            INSERT INTO invoices (invoice_number, customer_id, customer_name, customer_phone, total, discount, final_total, payment_method, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (invoice_number, customer['id'], customer['name'], customer['phone'], total, discount, final_total, payment_method, session['user_id']))
        cur.connection.commit()
        invoice_id = cur.lastrowid
        cur.close()
    # إضافة بنود وتحديث المخزون
    for item in cart:
    ...
    # تحديث نقاط العميل إذا كان مسجلاً
    if customer['id']:
...
if __name__ == '__main__':
    print("="*70)
    print("🚀 نظام إدارة السوبر ماركت - سوبر ماركت اولاد قايد محمد")
    print("="*70)
